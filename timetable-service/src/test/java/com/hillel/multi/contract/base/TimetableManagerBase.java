...
public class TimetableManagerBase {
    ...
    public void setUp() {
        ...
        Mockito.doThrow(new ResponseStatusException(HttpStatus.BAD_REQUEST)).when(timetableManagerService).addLesson(negativeLessonDTO);
        Mockito.doReturn(positiveLessonDTO).when(timetableManagerService).updateLesson(3, positiveLessonDTO);
        Mockito.doThrow(new ResponseStatusException(HttpStatus.NOT_FOUND)).when(timetableManagerService).updateLesson(-1, positiveLessonDTO);
        Mockito.doNothing().when(timetableManagerService).deleteLesson(3);
        Mockito.doThrow(new ResponseStatusException(HttpStatus.NOT_FOUND)).when(timetableManagerService).deleteLesson(-1);
